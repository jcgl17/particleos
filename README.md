...
since I discovered IPv6. Concretely, it's a configuration for systemd's
[`mkosi` tool](https://mkosi.systemd.io/) that you use to build your own
ParticleOS images. As described in the readme:
...
- OS versions are delivered as immutable `/usr` partitions
  ...
  - Authenticity provided by a signature on the dm-verity data, done with
    your own keys
- Is easily hacked on, just like traditional mutable OSes
  - Essentially *is* a traditional OS (one of Arch, Debian, or Fedora), built
    fully from regular distro packages
  - Hacking on `/usr` is done at image build-time, rather than during OS
    runtime
- Is signed with your own SecureBoot keys
- LUKS-encrypted root partition with TPM-stored key for automatic unlocking
